...
class _NewsScreenState extends State<NewsScreen> {
  ...
  void initState() {
    ...
    databaseService.getNews().then((List<News> newsList) {
      setState(() {
        newsDatabase = newsList;
      });
    });
  }
                              ...
                              Card(
                                ...
                                child: Padding(
                                  ...
                                  child: news.containQuotedText()
                                      ...
                                      : Text(
                                          news.text[LocalStorage.getLingua()],
                                          style: GoogleFonts.roboto(
                                            color: kBlack,
                                            height: 1.4,
                                            letterSpacing: .6,
                          ...
                          Stack(
                            children: [
                              Container(
                                width: MediaQuery.of(context).size.width,
                                height:
                                    MediaQuery.of(context).size.width * 11 / 16,
                                ...
                                children: [
                                  ...
                                  ClipRRect(
                                    child: GalleryImage(
                                      titleGallery:
                                          kMedia[LocalStorage.getLingua()],
                                      imageUrls: news.multi_imgs_link,
                                    ),
                                  ),
                                ],
                              ),
                            ...
                            child: Column(
                              children: [
                                ...
                                Card(
                                  color: kWhite,
                                  margin:
                                      EdgeInsets.fromLTRB(4.0, 4.0, 4.0, 30.0),
                                  child: Container(
                                    width: MediaQuery.of(context).size.width,
  ...
  List<Widget> getRelatedNews({required String language, required News news}) {
    ...
    for (int i = 0; i < newsDatabase.length; i++) {
      if (newsDatabase[i].tag['en'].toString().contains(news.tag['en'][0])) {
        if (j < 3) {
          relatedNewsWidgetsList.insert(
              j, getRelatedNewsWidget(newsDatabase[i]));
          i++;
        }
      }
      if (newsDatabase[i].tag['en'].toString().contains(news.tag['en'][1])) {
        if (k < 6) {
          relatedNewsWidgetsList.insert(
              k, getRelatedNewsWidget(newsDatabase[i]));
          i++;
        }
      }
    }
